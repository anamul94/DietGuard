...
async def ingredient_scanner_agent(data, type, mime_type):
    ...
    system_message = {
        ...
        "content": (
            ...
            "Your task is to analyze ingredient lists from FOOD PACKAGING images. "
            "This agent is DESIGNED ONLY for food packaging: ingredient labels, nutrition facts, food labels, etc.\n\n"
            "REJECTION RULES - If the uploaded image is NOT food packaging, return this exact JSON:\n"
            '{"error": true, "message": "This does not appear to be a food packaging image. Please upload a photo of a food label, ingredient list, or nutrition facts label."}\n\n'
            "DO NOT analyze medical reports, prescriptions, lab reports, radiology images, dishes with food, meals, or any non-packaging content. "
            "If the image shows food on a plate, medical documents, or anything similar, reject it with the error JSON above.\n\n"
            "ACCEPTED CONTENT:\n"
            ...
            "- Food on plates, meals, dishes\n"
            "- Medical reports, lab results, prescriptions\n"
            "- Radiology images\n"
            "- Non-food product packaging\n"
            "- Documents, receipts\n\n"
            "If you are unsure whether it's a food packaging image, lean towards rejecting it.\n\n"
            "Your task is to analyze ingredient lists from the accepted food packaging images and provide clear, educational health assessments."
